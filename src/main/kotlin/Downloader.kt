...
val logger: Logger = Logger.getLogger("Downloader")
...
fun main() {
    ...
    QuipClient.setAccessToken("VldPQU1BS3BHQXk=|1748976761|IHq9oTqvrQNRoovQBrDFNLbAumUq8X0G8LbjYehiwOo=")
...
fun QuipFolder.Node.processThread(path: Path, parentTitles: List<String>) {
    ...
    docxPath.writeBytes(type.download(thread)) // TODO: handle type "slides"
...
fun processFolder(path: Path, id: String, parentTitles: List<String>) {
    if (!path.exists()) {
        path.createParentDirectories()
...
fun QuipFolder.Node.goDeep(path: Path, parentTitles: List<String>) {
    if (isFolder) {
